...
class paint:	
	...
	def autoplot(self,r0=250, g0=0, b0=215):
		a,b = self.center_x, self.center_y
		self.mouse.move(1,100)
		time.sleep(1)
		im=grab()
		for j in range(100):
			left=-1
			for i in range(400):
				r1,g1,b1=im.getpixel((a-200+i,b-200+4*j))
				if max(abs(r1-r0),abs(g1-g0),abs(b1-b0))<30 and i<399:
					if left==-1:
						left = i
				else:
					if left != -1:
						self.drawline(a-200+left,b-200+4*j,a-200+i-1,b-200+4*j)
						left=-1
						
	def SilverBullet(self):
		h_c=0
		v_c=0
		print('This command will automatically destroy the pink color from left to right.')
		print('Please zoom in to a place on the left of the ball and set pen size between 1-4, then press any key to start.')
		input()
		self.setcolor(255,255,255)
		while True:
			h_c+=1
